...
import {
  GIT_OID, SAFE_ID, SAFE_MEDIA, SAFE_VERSION, SHA_256, boundedStringArray, enumValue, exactKeys,
  fail, portableRepository, record, requiredString,
} from "./common.ts";
...
export function validateSource(value: unknown): RuntimeSourceRef {
  ...
  if (kind === "git") {
    ...
    return { kind, repository, commit: requiredString(source.commit, "source.commit", GIT_OID, 40), tree: requiredString(source.tree, "source.tree", GIT_OID, 40) };
  }
  exactKeys(source, ["kind", "sha256", "mediaType"], "source");
  return { kind, sha256: requiredString(source.sha256, "source.sha256", SHA_256, 64), mediaType: requiredString(source.mediaType, "source.mediaType", SAFE_MEDIA, 255) };
}

function validateSubject<K extends string>(value: unknown, name: string, kinds: readonly K[]): RuntimeImmutableSubject<K> {
...
export function validateRuntimeProviderSubject(value: unknown, name = "providerSubject"): RuntimeProviderSubject {
  return validateSubject(value, name, ["source", "artifact", "binary"] as const);
}
export function validateRuntimeEnvironmentSubject(value: unknown, name = "environmentSubject"): RuntimeEnvironmentSubject {
  return validateSubject(value, name, ["image", "template", "profile"] as const);
}
...
export function validateRuntimeProviderDescriptor(value: unknown): RuntimeProviderDescriptor {
  ...
  exactKeys(descriptor, ["id", "version", "subject", "environment", "scope", "capabilities", "credentialBoundary", "implementation", "availability", "liveEvidence"], "providerDescriptor");
  ...
  const environment = validateRuntimeEnvironmentSubject(descriptor.environment, "providerDescriptor.environment");
  if (subject.id !== id || subject.version !== version) fail("providerDescriptor subject must bind id and version");
  const result: RuntimeProviderDescriptor = {
    id, version, subject, environment,
    scope: enumValue(descriptor.scope, "providerDescriptor.scope", ["local", "cloud"] as const),
    capabilities,
    credentialBoundary: enumValue(descriptor.credentialBoundary, "providerDescriptor.credentialBoundary", ["none", "host-only", "broker-only"] as const),
    implementation: enumValue(descriptor.implementation, "providerDescriptor.implementation", ["IMPLEMENTED", "NOT_IMPLEMENTED"] as const),
    availability: enumValue(descriptor.availability, "providerDescriptor.availability", ["AVAILABLE", "ABSENT", "REFUSED_POLICY"] as const),
    liveEvidence: enumValue(descriptor.liveEvidence, "providerDescriptor.liveEvidence", ["PASS", "FAIL", "NOT_EXERCISED"] as const),
  };
  if (result.implementation === "NOT_IMPLEMENTED" && result.liveEvidence !== "NOT_EXERCISED") fail("NOT_IMPLEMENTED provider cannot have live evidence");
  if (result.availability !== "AVAILABLE" && result.liveEvidence === "PASS") fail("unavailable provider cannot have PASS live evidence");
  return result;
}
