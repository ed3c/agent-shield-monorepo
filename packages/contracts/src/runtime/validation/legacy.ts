...
export function validateRuntimeRequest(value: unknown): RuntimeRequest {
  ...
  const legacyExclusion = "legacy-runtime-v1-unbound";
  const exclusions = Array.isArray(candidate.exclusions)
    ? [...new Set([...candidate.exclusions, legacyExclusion])]
    : candidate.exclusions;
  return validateRuntimeRequestV2({
    ...candidate,
    schema: RUNTIME_REQUEST_SCHEMA,
    providerVersion: "legacy-v1-unbound",
    providerSubject: { kind: "source", id: providerId, version: "legacy-v1-unbound", sha256: LEGACY_PROVIDER_SHA256 },
    environmentSubject: { kind: "profile", id: "legacy-v1-unbound", version: "1.0.0", sha256: LEGACY_ENVIRONMENT_SHA256 },
    exclusions,
  });
}
