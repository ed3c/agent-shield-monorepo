...
export function requiredString(value: unknown, name: string, pattern?: RegExp, maxLength = 1024): string {
  if (typeof value !== "string" || value.length === 0 || value.length > maxLength) fail(`${name} must be a non-empty bounded string`);
...
export function relativePath(value: string, name: string): void {
  if (value.startsWith("/") || value.startsWith("~") || /^[A-Za-z]:/.test(value) || value.includes("\\") || value.length > 255) {
...
export function portableRepository(value: string, name: string): void {
  let parsed: URL;
  try { parsed = new URL(value); } catch { fail(`${name} must be an absolute repository URL`); }
