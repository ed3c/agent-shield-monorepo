...
export type BoundedStageResult<T> =
...
export async function runBoundedStage<T>(
  ...
  const first = await Promise.race([
    settled,
    abortPromise.then((kind) => ({ kind } as const)),
  ]);
  ...
  const graceExpired = new Promise<"GRACE_EXPIRED">((resolve) => {
    graceHandle = setTimeout(() => resolve("GRACE_EXPIRED"), cancellationGraceMs);
  });
