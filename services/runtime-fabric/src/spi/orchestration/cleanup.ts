import { type RuntimeCleanupReceipt, type RuntimeOutcomeState, type RuntimeRequest } from "../../../../../packages/contracts/src/runtime/index.ts";
...
export async function recoveryCleanup(
  ...
  if (result.kind === "RESOLVED") {
    try {
      cleanup = normalizeCleanup(result.value, request, taskOutcome, "recovery");
    } catch {
      cleanup = failedCleanup("provider recovery cleanup returned an invalid receipt", ["recovery-cleanup-invalid"]);
    }
  } else if (result.kind === "REJECTED") {
    cleanup = failedCleanup("provider recovery cleanup threw", ["recovery-cleanup-exception"]);
  } else {
    cleanup = failedCleanup("provider recovery cleanup timed out", ["recovery-cleanup-timeout"]);
  }
...
export async function materializedCleanup(
  ...
  if (result.kind === "RESOLVED") {
    try {
      cleanup = normalizeCleanup(result.value, request, taskOutcome, "materialized");
    } catch {
      cleanup = failedCleanup("provider cleanup returned an invalid receipt", ["cleanup-invalid"]);
    }
  } else if (result.kind === "REJECTED") {
    cleanup = failedCleanup("provider cleanup threw", ["cleanup-exception"]);
  } else {
    cleanup = failedCleanup("provider cleanup timed out", ["cleanup-timeout"]);
  }
