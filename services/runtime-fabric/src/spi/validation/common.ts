...
export const RECEIPT_KEYS = [
  "schema", "requestId", "requestDigest", "provider", "source", "workspaceIdentity", "lifecycle",
  "taskStage", "terminalStage", "admission", "taskOutcome", "outcome", "state", "exit", "output",
  "artifacts", "touchedPaths", "cleanup", "exclusions", "detail",
] as const;
...
export function nonNegativeInteger(value: unknown, name: string, max: number): number {
  if (typeof value !== "number" || !Number.isSafeInteger(value) || value < 0 || value > max) throw new Error(`${name} must be an integer between 0 and ${max}`);
  return value;
}
...
export function unexercisedCleanup(detail: string): RuntimeCleanupReceipt {
  return {
    state: "NOT_EXERCISED", durationMs: 0, processesChecked: false, workspaceChecked: false, sessionsChecked: false,
    workspaceDisposition: "ABSENT", preservationRef: null, residue: [], detail,
  };
}
export function failedCleanup(detail: string, residue: string[] = ["cleanup-failed"]): RuntimeCleanupReceipt {
  return {
    state: "FAIL", durationMs: 0, processesChecked: false, workspaceChecked: false, sessionsChecked: false,
    workspaceDisposition: "UNKNOWN", preservationRef: null, residue, detail,
  };
}
