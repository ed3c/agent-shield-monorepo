...
export function normalizeCollection(value: unknown, request: RuntimeRequest): RuntimeCollectionResult {
  ...
  if (!Array.isArray(collection.touchedPaths) || collection.touchedPaths.length > request.limits.maxTouchedPaths) throw new Error("provider touched too many paths");
  ...
  for (const path of touchedPaths) if (!request.mutation.writableRoots.some((root) => path === root || path.startsWith(`${root}/`))) {
    throw new Error("provider touched a path outside mutation.writableRoots");
  }
  if (!Array.isArray(collection.artifacts) || collection.artifacts.length > request.artifacts.length) throw new Error("provider returned too many artifacts");
  ...
  if (state === "PASS") for (const required of request.artifacts.filter((entry) => entry.required)) {
    if (!observedKinds.has(required.kind)) throw new Error(`required artifact is missing: ${required.kind}`);
  }
  return { state, artifacts, touchedPaths, detail: portableDetail(collection.detail, "runtime collection result.detail") };
}
...
export function normalizeCleanup(
  ...
  exactKeys(cleanup, [
    "state", "durationMs", "processesChecked", "workspaceChecked", "sessionsChecked", "workspaceDisposition",
    "preservationRef", "residue", "detail",
  ...
  const workspaceDisposition = enumValue(cleanup.workspaceDisposition, "runtime cleanup receipt.workspaceDisposition", ["DELETED", "PRESERVED_BY_POLICY", "ABSENT", "UNKNOWN"] as const);
  const preservationRef = cleanup.preservationRef === null ? null : normalizeArtifactRef(cleanup.preservationRef, "runtime cleanup receipt.preservationRef", request.limits.maxArtifactBytes);
  ...
  if (state === "PASS") {
    ...
    if (mode === "materialized" && workspaceDisposition === "ABSENT") throw new Error("materialized workspace cannot be ABSENT");
  ...
  if (state === "NOT_EXERCISED" && (
    ...
    preservationRef !== null || residue.length > 0
  )) throw new Error("NOT_EXERCISED cleanup contains exercised state");
  return { state, durationMs, processesChecked, workspaceChecked, sessionsChecked, workspaceDisposition, preservationRef, residue, detail: portableDetail(cleanup.detail, "runtime cleanup receipt.detail") };
}
