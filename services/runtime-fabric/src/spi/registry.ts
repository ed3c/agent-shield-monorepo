...
export class RuntimeProviderRegistry {
  ...
  register(provider: RuntimeProviderSpi): void {
    ...
    if (this.#providers.has(descriptor.id)) throw new Error(`runtime provider is already registered: ${descriptor.id}`);
  ...
  resolve(providerId: string, scope: RuntimeRequest["scope"]): RuntimeProviderSpi | null {
    const provider = this.#providers.get(providerId) ?? null;
    if (provider && provider.descriptor.scope !== scope) throw new Error("registered provider scope does not match request scope");
    return provider;
  }
