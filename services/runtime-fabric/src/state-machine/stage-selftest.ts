...
function boundedRequest(timeoutMs = 30, cancellationGraceMs = 10): RuntimeRequest {
  ...
  value.cleanup = {
    ...(value.cleanup as Record<string, unknown>),
    maxDurationMs: 30,
  };
...
export async function runtimeStageSelftest(): Promise<void> {
  ...
  ok(
    preCancelledReceipt.taskStage === "ADMISSION" &&
    preCancelledReceipt.taskOutcome === "CANCELLED" &&
    preCancelledProvider.admitCalled === 0 &&
    preCancelledReceipt.cleanup.state === "NOT_EXERCISED",
  ...
  ok(
    materializationTimeout.taskStage === "MATERIALIZATION" &&
    materializationTimeout.taskOutcome === "TIMED_OUT" &&
    materializationTimeout.outcome === "TIMED_OUT" &&
    materializationTimeout.workspaceIdentity === null &&
    materializationTimeoutProvider.recoveryCleanupCalled === 1 &&
    materializationTimeout.cleanup.state === "PASS" &&
    materializationTimeout.exit.timedOut === false,
  ...
  executionTimeoutProvider.execute = async (_materialization, _request, context) => {
    ...
    return {
      ...
      stdoutBytes: 0,
      stderrBytes: 0,
      detail: "cooperative execution timeout",
    };
  };
  ...
  ok(
    ...
    executionTimeout.exit.timedOut && executionTimeoutProvider.cleanupCalled === 1 &&
    executionTimeout.cleanup.state === "PASS",
  ...
  collectionTimeoutProvider.collect = async (_materialization, _request, _execution, context) => {
  ...
  ok(
    ...
    collectionTimeout.exit.code === 0 && !collectionTimeout.exit.timedOut &&
    collectionTimeout.cleanup.state === "PASS",
  ...
  cleanupTimeoutProvider.cleanup = async (_materialization, _request, _taskOutcome, context) => {
    ...
    return {
      ...
      residue: ["cleanup-timeout"],
      detail: "cooperative cleanup timeout",
    };
  };
  ...
  ok(
    ...
    uncooperative.outcome === "FAILED_CLEANUP" &&
    uncooperative.cleanup.residue.includes("execution-operation-unsettled") &&
  ...
  cancellationProvider.execute = async (_materialization, _request, context) => {
    cancellationProvider.executeCalled += 1;
    await waitForAbort(context.signal);
    return {
      state: "CANCELLED",
      exit: { code: null, signal: null, timedOut: false, cancelled: true },
      stdoutBytes: 0,
      stderrBytes: 0,
      detail: "cooperative execution cancellation",
    };
  };
  ...
  ok(
    cancelled.taskStage === "EXECUTION" && cancelled.taskOutcome === "CANCELLED" &&
    cancelled.exit.cancelled && cancelled.cleanup.state === "PASS",
