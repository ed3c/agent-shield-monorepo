...
export function validateLocalFixtureReceipt(receipt) {
  assert(
...
export function runReversibleLocalFixture(contract, now) {
  ...
  const probeScript = join(HERE, "local-fixture-probe.js");
  let childExit = null;
  let stdout = "";
  ...
  try {
    ...
    const child = spawnSync(process.execPath, [probeScript], {
      cwd: workspace,
      env: {
        LANG: "C.UTF-8",
        LC_ALL: "C.UTF-8",
    ...
    assert(
      Buffer.byteLength(stdout) + Buffer.byteLength(stderr) <=
        contract.workload.resources.outputBytes,
      "local fixture output exceeded the contract bound",
    );

    artifact = readFileSync(join(workspace, "probe-output.json"));
