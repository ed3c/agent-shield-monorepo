...
function withDigest<T extends Record<string, unknown>>(value: T): T & { content_sha256: string } {
  return { ...value, content_sha256: sha(canonical(value)) };
}

const root = resolve(".");
const modules = (readdirSync(join(root, ".arena/modules")) as string[]).sort().map((id: string) => {
  const path = join(root, ".arena/modules", id, "module.json");
...
const target = arg >= 0 && process.argv[arg + 1] ? resolve(process.argv[arg + 1]) : join(root, "data/releases/agent-shield-module-set.json");
